...
from app.run.models import PreviewSettings, RunSpec
...
def configured_run_spec(
    ...
    *,
    ...
    """Return the explicit run spec for one local application execution."""
    return RunSpec(
...
def configured_check_profile_name() -> str | None:
    """Return the selected application check profile, if explicitly configured."""
...
def configured_source_snapshot_path(project_root: Path) -> Path:
    ...
    if configured is None or not configured.strip():
        raise ValueError(
            "SOURCE_SNAPSHOT_PATH must be set for local runtime runs. Use the demo image for the bundled example snapshot."
        )
    return Path(configured).expanduser().resolve()


def configured_parity_store_path(project_root: Path) -> Path:
    """Return the configured application-owned run store path."""
