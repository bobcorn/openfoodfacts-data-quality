...
# About migration runs

A migration run starts from a full product source snapshot and ends with
...
line. Rows with a missing or blank product code are skipped before execution,
logged as warnings, and summarized in the report and `run.json`. Unsupported
formats still fail before batch execution starts.
...
`MigrationRunner` resets `artifacts/latest/`, executes the prepared run, and
builds the final
