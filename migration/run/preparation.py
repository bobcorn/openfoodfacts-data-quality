...
from migration.source.datasets import load_dataset_profile
...
def prepare_run(
    ...
    *,
    ...
    source_input_summary = summarize_source_input(
        run_spec.db_path,
        selection=active_dataset_profile.selection,
    )
    product_count = source_input_summary.processed_product_count
    ...
    if source_input_summary.skipped_row_count:
        logger.warning(
            "[Input] Skipped %d source row(s) with invalid product codes before execution.",
